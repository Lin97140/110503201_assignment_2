...
int isoccupied(stack *stackptr, player *me, player *enemy, int position_x, int position_y, int kind)
{
    int type;
...
int move(player *me, player *enemy, int kind, int position_x, int position_y, stack *stackptr)
{
...
int go_back(stack *ptr, player **game, int *chess, int *position_x, int *position_y)
{
    int player = 0;
    if (ptr->top == 0) //若目前只有下一手
    {
        if (ptr->name[(ptr->top)] == 'y') // pop一次
        {
            player = 1;
        }
        pop(ptr, chess, &(game[player]->position[*chess][0]), &(game[player]->position[*chess][1]));
        return 0;
    }
    ...
    else
    {
        for (int i = 0; i < 2; i++)
        {
            if (ptr->die[(ptr->top)]) //若遇到標註為死掉的棋，多做一次把被吃掉的棋復原
            {
                player = 0;
                if (ptr->name[(ptr->top)] == 'y')
                {
                    player = 1;
                }
                pop(ptr, chess, &(game[player]->position[(ptr->chess[(ptr->top)])][0]), &(game[player]->position[(ptr->chess[(ptr->top)])][1]));
            }
            player = 0;
...
void save_file(stack *stackptr, FILE **fptr)
{
    int top = stackptr->top;
    while (top != -1)
    {
        //將stack內容寫入遊戲檔案
        fprintf(*fptr, "%c %d %d %d %d %d %d\n", stackptr->name[top], stackptr->chess[top], stackptr->original_x[top], stackptr->original_y[top], stackptr->position_x[top], stackptr->position_y[top], stackptr->die[top]);
        top--;
    }
}

void load_file(stack *stackptr, FILE **fptr)
{
    char name, enter;
...
int ask_oldgame(stack *ptr, player **game, int *index, player *x, player *y)
{
    char action;
    ...
    if (action == 'f')
    {
        if (*index != -1)
        {
            for (int i = 0; i < 2; i++) //預設改變兩手
            {
                ...
                game[player]->position[ptr->chess[*index]][0] = ptr->position_x[*index];
                game[player]->position[ptr->chess[*index]][1] = ptr->position_y[*index];
                (*index)--;
                if (!ptr->die[*index] || *index == -1) //若沒有遇到標註為死掉的棋，只改變一手棋
                {
                    break;
                }
            }
    ...
    else if (action == 'b')
    {
        if (*index + 1 <= ptr->top)
        {
            ...
            game[player]->position[ptr->chess[*index]][0] = ptr->original_x[*index];
            game[player]->position[ptr->chess[*index]][1] = ptr->original_y[*index];
            if (ptr->die[*index])
            {
                (*index)++;
                if (ptr->name[*index] == 'y')
...
int main(int argc, char *argv[])
{
        ...
        while (1)
        {
            ...
            do
            {
                ...
                if (!chess)
                {
                    ...
                    state = go_back(&store, &game[0], &chess_before, &position_x_before, &position_y_before); //悔棋
                    if (!state)
                    {
                        break;
                    }
            ...
            } while (!move(me, enemy, chess, position_x, position_y, &store) || chess == -1); // 當移動方式符合規定
            if (chess == -2)
            {
                break;
            }
    ...
    else if (argc == 3)
    {
        ...
        fptr = fopen(path, "r");
        load_file(&store, &fptr);
        index = store.top;
        show(&x);
        show(&y);
        while (flag)
        {
            flag = ask_oldgame(&store, game, &index, &x, &y); //回復舊檔
        }
    }
    fclose(fptr);
    return 0;
